...
import { AlertCircle, Construction, TrafficCone } from "lucide-react";
                ...
                <CardContent className="space-y-4">
                    {roadStatsData.bridges.map((bridge, index) => (
                         <div key={index} className="flex items-center gap-4 p-3 rounded-lg border">
                            <div className={`h-3 w-3 rounded-full ${bridge.ok ? 'bg-success' : 'bg-destructive'}`} />
                            <div>
                                <p className="font-semibold">{bridge.name}</p>
                                <p className={`text-sm ${bridge.ok ? 'text-muted-foreground' : 'text-destructive'}`}>{bridge.status}</p>
                            </div>
                        </div>
                    ))}
                </CardContent>
             ...
             <Card className="md:col-span-2">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        ...
                        Rues Endommagées à Éviter
                    </CardTitle>
                    <CardDescription>Mises à jour sur les rues nécessitant des réparations.</CardDescription>
                </CardHeader>
                <CardContent>
